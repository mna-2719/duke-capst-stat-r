...
```{r Q2}
is.inte <- sapply(ames_train, is.integer) #return a logical vector
integs.df <- ames_train[, is.inte] #extract these columns
lapply(integs.df, levels) #get the levels
```
...
```{r}
#change `Overall.Qual` and `Overall.Cond` to categorical variable
...
ames_train %>%
  ...
  summarise(mean_dd = mean(price), sd_dd = sd(price), n = n())
...
```{r Q4}
ggplot(ames_train, aes(x=price, y=Lot.Area)) +
  geom_point() +
  geom_smooth(method=lm)

ggplot(ames_train, aes(x=price, y=Bedroom.AbvGr)) +
  geom_point() +
  geom_smooth(method=lm)

ggplot(ames_train, aes(x=price, y=Overall.Qual)) +
  geom_point() +
  geom_smooth(method=lm)

ggplot(ames_train, aes(x=price, y=Year.Built)) +
  geom_point() +
  geom_smooth(method=lm)
...
```{r Q5}
ggplot(ames_train, aes(x=price)) + geom_density()

ggplot(ames_train, aes(x=area)) + geom_density()
...
ames_train$price10<-log10(ames_train$price)
ames_train$area10<-log10(ames_train$area)

ggplot(ames_train, aes(x=price10)) + geom_density()

ggplot(ames_train, aes(x=area10)) + geom_density()

ggplot(ames_train, aes(x=price10, y=area10)) +
  geom_point() +
  geom_smooth(method=lm)
